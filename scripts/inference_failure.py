...
def parse_args():
    ...
    p.add_argument("--checkpoint", type=str, default=None, help="Optional. Path to trained weights.")
    p.add_argument("--input", type=str, default="./data/inference/photo4.jpg", help="Single input path.")
    p.add_argument("--output", type=str, default="./inference_output.png", help="Single output path.")
    p.add_argument("--input_dir", type=str, default=None, help="Batch: source image dir.")
    p.add_argument("--output_dir", type=str, default=None, help="Batch: output dir.")
    p.add_argument("--organize", action="store_true", help="Auto-organize real/ and gen/ dirs.")
    
    # --- Core inference args ---
    p.add_argument("--t0", type=float, default=0.35, help="ODE start step. 0.5=strong style but unstable, 0.25=faithful; try 0.3-0.4.")
    p.add_argument("--steps", type=int, default=25, help="ODE steps. More steps = smoother Euler.")
    p.add_argument("--style_strength", type=float, default=0.6, help="Style strength scale. Lower (e.g. 0.5) can fix red shift/noise.")
    p.add_argument("--clamp_val", type=float, default=1.2, help="Clamp for v_pred. Tighter (0.8-1.2) can prevent color overflow.")
    
    p.add_argument("--single_step", action="store_true", help="Single-step mode (like training Preview).")
...
def _run_one(model, device, img_tensor, t0, steps, guidance, style_strength=1.0, clamp_val=3.0, single_step=False, single_step_t=0.2):
    """Optimized inference core: strength scaling and dynamic clamping."""
    ...
    with torch.no_grad():
        ...
        if single_step:
            ...
            # Apply strength scale and clamp
            ...
            for i in range(steps):
                ...
                v_pred = model.unet(**unet_kw)[0]
                # Scale v_pred to reduce accumulated red shift
                v_pred = v_pred * style_strength
                v_pred = torch.nan_to_num(v_pred, nan=0.0).clamp(-clamp_val, clamp_val)
                x_t = x_t - v_pred * dt
